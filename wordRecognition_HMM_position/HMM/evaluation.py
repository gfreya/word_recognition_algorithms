...
if __name__ =='__main__':
    ...
    #得到textdata的字符串个数
    with open('../HMMdata/testsequence.txt') as f:
        data = f.read()
    allSample = len(data)
    print (allSample)
    ...
    if flag == 1:  # 将比较结果以html格式输出
        compMethodOne('../HMMdata/testPosition.txt', 'bigOut.txt', '../compareResult/bigcompMethodOne.html')
    elif flag == 2:  # 将比较结果输出到txt文件中
        comMethodTwo('../HMMdata/testPosition.txt', 'bigOut.txt', '../compareResult/bigcompMethodTwo.txt')
    else:
        print("Wrong option!!!")
    '''
